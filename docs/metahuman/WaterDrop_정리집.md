...
```
new_r(t) = sqrt((x₀ − R·cos t)^2 + (y₀ − R·sin t)^2)
x(t) = new_r(t) · R · cos(t)
y(t) = R · sin(t)
```
...
```
x = 0.5 · (1 − cos θ) · sin θ · cos \phi
y = 0.5 · (1 − cos θ) · sin θ · sin \phi
z = cos θ
```
...
x = horizontalR · cos \phi
y = cos θ                    ← 높이
z = horizontalR · sin \phi
```
...
|---|---|---|
| 출처 | `(1 − cos θ) · sin θ` | `x⁴ − x³ + y^2 + z^2 = 0` |
...
| 단면 반경 함수 | `(1−cos θ)·sin θ` | `\sqrt(t³(1−t))` |
| 최대 반경 위치 | θ ≈ 2.2 rad | t = 0.75 (비대칭) |

WaterDrop은 위아래가 대칭인 진짜 눈물방울이고, Piriform은 한쪽만 뾰족한 서양 배 — `(x³)(1−x)`의 부호 영역이 `x ∈ [0, 1]`로 닫혀 있어서 **자연스럽게 한쪽만 뾰족**해진다. 일반화 형태 `(x⁴ − a·x³) + a^2(y^2 + z^2) = 0`으로 쓰면 `a` 파라미터로 비례를 조절할 수 있다 (Bourke 식은 `a = 1`인 특수 경우).
...
음함수 `x⁴ − x³ + y^2 + z^2 = 0`을 GeoGebra 3D에 직접 입력하면 거의 안 그려진다 — GeoGebra의 **음함수 곡면 렌더링이 매우 빈약**하기 때문이다. 반드시 `Surface(...)` 명령으로 매개변수 형태를 줘야 한다.
...
WaterDrop의 단면 함수는 [0, π]에서 거의 대칭이지만, piriform의 `\sqrt(t³(1−t))`는 **최대점이 t = 0.75에 치우쳐** 있다. 따라서 `t`를 균등 분할하면:
...
- Paul Bourke. *Piriform tear drop* (2003). `x⁴ − x³ + y^2 + z^2 = 0`. (7장)
- MathWorld. *Piriform Curve / Piriform Surface*. 일반화 형식 `(x⁴ − a·x³) + a^2(y^2 + z^2) = 0`.
...
class PiriformY : public ParametricGeometry
{
    ...
    // r(t) = \sqrt(t³(1−t)),  t ∈ [0.05, 0.98] 권장 (양 끝 cusp/degenerate 회피)
public:
    ...
    virtual vmath::vec4 SurfaceFunction(double u, double v) const override
    {
        // u 를 t 로 사용 (회전축 위치)
        float t = (float)u;
        float r = sqrtf(t * t * t * (1.0f - t));   // = \sqrt(t³(1−t))
        float phi = (float)v;
