...
##### 동일한 $u$, $v$ 가 들어오더라도, 모델의 모양은 천차만별 <br> $ x = f_x(u, v) \quad y = f_y(u, v) \quad z = f_z(u, v) \quad$
...
1. $u$ & $v$ 
   ...
   2. 얼마나 잘개 쪼갤것 (iterate)할 지. 해상도 값 입력
        * $u_{res} \in [\text{0}, \infty)$
        * $v_{res} \in [\text{0}, \infty)$
         * 이게 곧 Quad 메쉬의 개수가 된다. 
...
3. **이터레이팅**
   1. **position** : $u_{start} + \Delta u * \text{IterCount}$
        * $u, v$ 별로 $\Delta u, \Delta v$ 만큼 한칸한칸씩 위상을 옮겨가면서
        정점 데이터를 쌓아나가면 된다.
...
$$
F(x, y, z) = 0 \quad \text{를 만족하는 해를 도형의 곡면으로 사용}
$$
...
##### 이렇게 함으로서, 심미적으로 올바른 위치에 텍스쳐를 배치할 수 있습니다.
* 여기서 파라메트릭 서피스의 장점이 드러나는데 
바로 **"Natural Parameterization"** 가 매우 손쉽다는점 입니다.
...
* UV 변환은 GL_TEXTURE 매트릭스 스텍에 적용한다.
  ...
  void Draw() override {
    ...
    glScalef(uv.scale.x, uv.scale.y, 1.0f); // UV의 넓게 펴바르는등. Ratio 조절
    ...
  }
  ```
...
##### 따라서 케로로의 모자를 추상적으로 생각해서, <br> 그림이 그려져야하는 부분, 그릴 필요없는 부분을 Aplha 값으로 제거하는 기법 선택
...
* 그러한 옵션을 활성화 하는 API 임.

##### ② `glAlphaFunc(GL_GREATER, 0.9f);`

* 어느상황에서 폐기해야 하나에 대한 함수를 설정할 수 있고,
* 이는 $ \alpha \gt 0.9$ 라면 색상 폐기하겠다는것을 설정.

##### ③ `glDisable(GL_ALPHA_TEST);`
* Model을 Draw하고나면 다른 GL Context의 오염을 막기 위해 명시적으로 해제해 줘야 합니다.
